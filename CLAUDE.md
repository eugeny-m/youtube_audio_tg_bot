...
- В локальном глобальном окружении код не исполняется: никаких `pip install`, `pytest`, `python ...`, экспериментов со скриптами. Тесты, эксперименты, проверки — только внутри Docker-контейнера проекта.
...
- **Client chain**: both stream discovery and download walk WEB → TV → ANDROID_VR (`FALLBACK_CLIENTS`), taking the first client that answers. WEB leads because only it exposes dubbed/localized tracks; discovery builds its SABR streams manually from `vid_info` (per-format, so one unparseable format doesn't sink the list), but download must take the stream from `yt.fmt_streams`: only that path applies signature/n-sig and po_token, and a manually built WEB URL answers HTTP 403. No single client is reliable on its own — YouTube intermittently answers `LOGIN_REQUIRED` ("not a bot") to whichever one it likes, hence the chain and the one retry on `BotDetection`. The two fallbacks are not interchangeable either: ANDROID_VR carries itag 139, TV carries 250.
...
- **Stateless service**: All `YoutubeService` methods are `@staticmethod` - no instance state. The download re-fetches the WEB stream by itag rather than reusing discovery's objects.
